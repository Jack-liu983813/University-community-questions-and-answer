...
(function() {
    ...
    _cfg.qn = {
        getUploadTokenApi: '../../api/getqiniuToken', //上传到指定目录
        getUploadTokenApi2: '../../api/getqiniuToken', //上传到随机文件名
                ...
                for (var i = 0; i < fileobjs.length; i++) {
                    ...
                    xhr = _fns.uploadFileQn2(res.data.uptoken, f,
                        ...
                        function (res) {
                            //把七牛的返回结果转为标准格式
                            res['success'] = true;
                            f.url = res.url = res['file_path'] = "http://opmgw7rhl.bkt.clouddn.com/" + res.key;
                            res['msg'] = 'upload ok.';
                            if (successfn) successfn(f, res);
                        },
                        function (f, err) {
